...
export default function EventsPage() {
  ...
  const { items, total } = useMemo(
    ...
    [allEvents, search, eventType, severity, sortBy, sortDir, page]
  );
    ...
    <AppShell connected systemStatus="HEALTHY">
      ...
      <Card>
        ...
        <CardContent className="grid gap-3 md:grid-cols-5">
          ...
          />
          <select className={fieldClass} value={eventType} onChange={(e) => { setPage(0); setEventType(e.target.value); }}>
            <option value="">All Types</option>
            {EVENT_TYPES.filter(Boolean).map((t) => <option key={t} value={t}>{t}</option>)}
          </select>
          <select className={fieldClass} value={severity} onChange={(e) => { setPage(0); setSeverity(e.target.value); }}>
      ...
      <div className="grid gap-4 xl:grid-cols-3">
        <Card className="overflow-hidden xl:col-span-2">
          <CardContent className="overflow-x-auto p-0">
            <table className="min-w-full text-left text-sm">
              <thead className="border-b border-white/10 bg-white/[0.03] text-[10px] uppercase tracking-[0.14em] text-slate-400">
                <tr>
                  {["Time", "Type", "Source", "Dest", "User", "Severity", "Risk", "Anomaly"].map((h) => (
              ...
              <tbody>
                {items.map((e) => (
                  ...
                  >
                    <td className="px-3 py-2 text-xs text-slate-400">{formatDateTime(e.timestamp)}</td>
                    ...
                    <td className="px-3 py-2"><Badge severity={e.severity}>{e.severity}</Badge></td>
                    <td className="px-3 py-2 font-mono text-violet-300">{Math.round(e.risk_score)}</td>
                    <td className="px-3 py-2 font-mono text-cyan-300">{Math.round(e.anomaly_score)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="flex items-center justify-between border-t border-white/10 px-4 py-3 text-sm">
            <span className="text-slate-500">{total} events · page {page + 1}/{pages}</span>
